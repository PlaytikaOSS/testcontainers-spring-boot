...
public class EmbeddedCockroachDBBootstrapConfigurationTest {
    ...
    public void shouldEmulateLatency() throws Exception {
        ...
        cockroachContainerProxy.toxics().latency("latency", ToxicDirection.DOWNSTREAM, 1000);

        assertThat(durationOf(() -> jdbcTemplate.queryForList("select name from operator", String.class)))
                .isCloseTo(1000L, Offset.offset(100L));

        cockroachContainerProxy.toxics().get("latency").remove();
    ...
    @Test
    public void shouldInitDBForCockroachDB() throws Exception {
    ...
    public void propertiesAreAvailable() {
        assertThat(environment.getProperty("embedded.cockroach.port")).isNotEmpty();
        assertThat(environment.getProperty("embedded.cockroach.host")).isNotEmpty();
        assertThat(environment.getProperty("embedded.cockroach.schema")).isNotEmpty();
        assertThat(environment.getProperty("embedded.cockroach.user")).isNotEmpty();
        assertThat(environment.getProperty("embedded.cockroach.password")).isEmpty();
        assertThat(environment.getProperty("embedded.cockroach.init-script-path")).isNotEmpty();
    }
